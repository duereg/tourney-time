...
var elimination = function (numTeams, p) {
  ...
  for (var match = 1; match <= Math.pow(2, p - 1); match += 1) {
    ...
    matches.push({ id: gId(p, 1, match), teams: woMark(maSeed, numTeams), isBye: isBye(maSeed, numTeams) });
  }
  ...
  for (round = 2; round <= p; round += 1) {
    for (game = 1; game <= Math.pow(2, p - round); game += 1) {
      ...
      matches.push({id: gId(p, round, game), teams: blank(matchOne, matchTwo, WB) });
    }
  ...
  if (matchOne && matchTwo && !!!matchOne.isBye && !!!matchTwo.isBye) {
    ...
    matches.splice(matches.length -1, 0, { id: gId(p, round - 1, 1), teams: blank(matchOne, matchTwo, LB) });
